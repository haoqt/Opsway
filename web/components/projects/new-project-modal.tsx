...
export function NewProjectModal({ onClose, onCreated }: Props) {
  ...
  const validate = () => {
    ...
    else if (!form.repo_full_name.includes("/")) errs.repo_full_name = "Must be in owner/repo format";
    setErrors(errs);
      ...
      <div className="w-full max-w-md rounded-2xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] shadow-2xl p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-3.5">
            ...
            <div className="grid grid-cols-3 gap-2">
              {[
                ...
                <button
                  key={value}
                  type="button"
                  onClick={() => setForm((f) => ({ ...f, git_provider: value }))}
          ...
          <Input
            ...
            hint="Format: owner/repo (e.g. mycompany/odoo-custom)"
          />
